...
pub fn update(p: &Platform, detail: &mut LoadingDetail) -> Option<State> {
    if detail.map_recv.is_none() {
        let (tx, rx) = channel();
        p.core.spawn(move |core| {
			let m = ::util::load_map(&core, "../assets/maps/city.tmx");
			tx.send(m).unwrap();
        });
        detail.map_recv = Some(rx);
    } else if detail.map.is_none() {
        match detail.map_recv.as_ref().unwrap().try_recv() {
            Ok(m) => detail.map = Some(m.into_map()),
    ...
    if detail.dot_timer >= detail.dot_delay {
        if detail.map.is_some() {
            // Fuck it, we're done here.
            return Some(State::GameMap(GameMapDetail{ map: detail.map.take().unwrap() }));
        }
